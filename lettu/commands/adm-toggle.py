...
async def adm_toggle(ctx: lightbulb.context.Context):
    ...
    if feature == "Birthdays":
        if EnabledBirthday == 0:
            cur.execute(
                "UPDATE GuildSettings SET EnabledBirthday = 1 WHERE GuildID = ?", (guild_id,))
            state = "are now enabled"
        else:
            cur.execute(
                "UPDATE GuildSettings SET EnabledBirthday = 0 WHERE GuildID = ?", (guild_id,))
            state = "are now disabled"
    elif feature == "Anniversaries":
        if EnabledAnniversary == 0:
            cur.execute(
                "UPDATE GuildSettings SET EnabledAnniversary = 1 WHERE GuildID = ?", (guild_id,))
            state = "are now enabled"
        else:
            cur.execute(
                "UPDATE GuildSettings SET EnabledAnniversary = 0 WHERE GuildID = ?", (guild_id,))
            state = "are now disabled"
    elif feature == "Autokick":
        if EnabledAutoKick == 0:
            cur.execute(
                "UPDATE GuildSettings SET EnabledAutoKick = 1 WHERE GuildID = ?", (guild_id,))
            state = "is now enabled"
        else:
            cur.execute(
                "UPDATE GuildSettings SET EnabledAutoKick = 0 WHERE GuildID = ?", (guild_id,))
            state = "is now disabled"
    else:
        if EnabledKickWarning == 0:
            cur.execute(
                "UPDATE GuildSettings SET EnabledKickWarning = 1 WHERE GuildID = ?", (guild_id,))
            state = "are now enabled"
        else:
            cur.execute(
                "UPDATE GuildSettings SET EnabledKickWarning = 0 WHERE GuildID = ?", (guild_id,))
            state = "are now disabled"
