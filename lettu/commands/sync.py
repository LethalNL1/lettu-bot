...
async def sync(ctx: lightbulb.context.Context):
    ...
    cur = db.connect()
    cur.execute(
        "SELECT Timezone, Birthday FROM Members WHERE MemberID = ? AND GuildID = ?", (member_id, guild_id))
    for timezone, birthday in cur:
        pass
    cur.execute("UPDATE Members SET Timezone = ?, Birthday = ? WHERE MemberID = ? AND GuildID != ?",
                (timezone, birthday, member_id, guild_id))
    cur.close()
